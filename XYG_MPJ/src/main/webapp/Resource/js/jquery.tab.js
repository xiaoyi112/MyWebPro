	...
	layui.use('element',function(){
		...
		var active={
		 ...
		 $('.site-demo-active').on('click', function() {
			var dataid=$(this);
			console.log(dataid)
			//判断当前标题下tab页数量是否大于0,如果大于0则打开该tab页,否则创建tab页
			if($('.layui-tab-title li[lay-id]').length<=0){
				console.log($('.layui-tab-title li[lay-id]').length);
				active.tabAdd(dataid.attr('data-url'),dataid.attr('data-title'),dataid.attr('data-id'));
			}else{
				//判断该tab项是否存在
				var isData=false;//初始化一个boolean变量，为false说明未打开该变量			
